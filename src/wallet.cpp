...
// Bitlectrum: optional setting to unlock wallet for block minting only;
...
// Bitlectrum: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Bitlectrum: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Bitlectrum: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Bitlectrum: not avatar mode
                        {
...
// Bitlectrum: create coin stake transaction
...
// Bitlectrum: check 'spent' consistency between wallet and txindex
// Bitlectrum: fix wallet spent state according to txindex
...
// Bitlectrum: disable transaction (only for coinstake)
