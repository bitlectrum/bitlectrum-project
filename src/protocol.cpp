...
// Bitlectrum message start (switch from Bitcoin's in v0.2)
static unsigned char pchMessageStartBitcoin[4] = { 0xec, 0xfb, 0xbc, 0xf9};
static unsigned char pchMessageStartBitlectrum[4] = { 0xfc, 0xf9, 0xb9, 0xf3 };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartBitlectrum : pchMessageStartBitcoin, sizeof(pchMessageStartBitlectrum));
}
