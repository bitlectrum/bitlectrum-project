...
static const int64 MAX_MINT_PROOF_OF_WORK = 25 * COIN;
...
public:
    ...
    {
        // Bitlectrum: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Bitlectrum: get transaction coin age
...
public:
    ...
    // Bitlectrum: block signature - signed by coin base txout[0]'s owner
    ...
    // Bitlectrum: two types of block: proof-of-work or proof-of-stake
    ...
    // Bitlectrum: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Bitlectrum: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Bitlectrum: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Bitlectrum: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Bitlectrum: block index flags
    enum  
    {
